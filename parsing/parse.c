...
{
	if (ft_strncmp(buf, "F ", 2) == 0 || ft_strncmp(buf, "C ", 2) == 0)
	{
		if (!get_colour(map, buf[0], &(buf[1])))
			return (0);
	}
	else if (ft_strncmp(buf, "NO ", 3) == 0 || \
		ft_strncmp(buf, "SO ", 3) == 0 || ft_strncmp(buf, "WE ", 3) == 0 || \
		ft_strncmp(buf, "EA ", 3) == 0)
	{
...
int	got_to_map(t_map *map, int fd, char *buf)
{
...
int	gather_data(t_map *map, char *av, int *lines)
{
	...
	while (buf)
	{
		...
		if (map->all_info != 6 && !first_read_till_map(map, buf))
			return (freebuf_closefd(buf, fd));
		else if (map->all_info == 6 && buf[0] != '\0' && mapline(buf))
		{
			if (!got_to_map(map, fd, buf))
				return (0);
			else
				break ;
		}
